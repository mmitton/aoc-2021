...
fn main() -> Result<(), Error> {
    ...
    const ITERS: usize = 50_000_000_000;
    ...
    for gen in 1..=ITERS {
        ...
        sums.push(sum(&states[cur]));
        if sums.len() > WINDOW {
            if let Some(delta) = last(&sums, WINDOW) {
                println!(
                    "Going up by {} every generation after {} generations",
                    delta, gen
                );
                let sum = sum(&states[cur]) + ((ITERS - gen) as isize * delta);
                println!("Sum: {:?}", sum);
                return Ok(());
            }
        }
