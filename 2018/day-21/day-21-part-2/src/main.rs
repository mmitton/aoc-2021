...
impl CPU {
    ...
    fn step(&mut self) {
        let inst = self.instructions[self.registers[self.pc_in]];
        if self.registers[self.pc_in] == 28 {
            println!(
                "ip={:02} {:9?} {:?} {} {} {} ",
...
fn main() -> Result<(), Error> {
    ...
    while cpu.running() {
        cpu.step();
        if cpu.registers[cpu.pc_in] == 28 {
            if seen.contains(&cpu.registers[5]) {
                println!("Answer: {}", last);
                return Ok(());
            }
            seen.push(cpu.registers[5]);
            last = cpu.registers[5];
        }
    }
