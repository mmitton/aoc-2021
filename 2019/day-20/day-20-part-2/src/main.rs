        ...
        macro_rules! add {
            ($pos:expr, $warp:expr) => {{
                ...
                if !seen.contains(&pos) {
                    seen.insert(pos);
                    let warp: Option<(&String, &Option<Coord>, &bool)> = $warp;
            ...
            ($delta:expr) => {{
                ...
                if pos != start && !job.path.contains(&pos) {
                    match map.get(&pos) {
                        ...
                        Some(Tile::Warp(from, inside, to)) => add!(pos, Some((from, to, inside))),
                        Some(Tile::Char(_)) => {}
                        None => unreachable!(),
                    }
                }
...
fn process(map: &mut BTreeMap<Coord, Tile>) {
    ...
    while let Some(job) = work.pop_first() {
        ...
        for path in paths.get(&pos).unwrap() {
            if job.level == 0 && !path.inside {
                if path.warp_to.is_none() {
                    // Found exit
                    let new_job = Job {
                        pos: path.warp_to,
                        level: 0,
            ...
            } else if path.warp_to.is_some() {
                let level = if path.inside {
                    job.level.checked_add(1).unwrap()
                } else {
                    job.level.checked_sub(1).unwrap()
                };
                let new_job = Job {
                    pos: path.warp_to,
                    level,
