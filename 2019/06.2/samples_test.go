...
func TestOrbitTransfers(t *testing.T) {
	...
	var testCases = []*TestCase{
		&TestCase{[]string{"COM)B", "B)C", "C)D", "D)E", "E)F", "B)G", "G)H", "D)I", "E)J", "J)K", "K)L", "K)YOU", "I)SAN"}, 4},
	}
	...
	for _, testCase := range testCases {
		...
		if err != nil {
			...
			path, err := com.findPath("YOU", "SAN")
			if err != nil {
				t.Errorf("ERROR: %v", err)
			} else if len(path) != testCase.expected {
				t.Errorf("ERROR: Def:%v  Path:%v  Expected:%v  Got:%v", testCase.defs, path, testCase.expected, len(path))
			} else {
				t.Logf("Def:%v  Path:%v  Expected:%v  Got:%v", testCase.defs, path, testCase.expected, len(path))
			}
		}
	}
}
