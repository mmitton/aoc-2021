...
func (ic *Intcode) lazyZero(addr int64) {
	for addr >= int64(len(ic.memory)) {
		ic.memory = append(ic.memory, 0)
	}
}
...
func (ic *Intcode) setDirect(addr int64, v int64) {
	ic.lazyZero(addr)
	ic.memory[addr] = v
}
...
func (ic *Intcode) getMemory(mode AccessMode, addr int64) int64 {
	switch mode {
	case ImmediateMode:
		return ic.getDirect(addr)
	case PositionMode:
		return ic.getDirect(ic.getDirect(addr))
	case RelativeMode:
		return ic.getDirect(ic.getDirect(addr) + ic.rb)
	case IndirectMode:
		return ic.getDirect(ic.getDirect(ic.getDirect(addr)))
	}
...
func (ic *Intcode) setMemory(mode AccessMode, addr int64, v int64) {
	switch mode {
	case PositionMode:
		ic.setDirect(ic.getDirect(addr), v)
	case RelativeMode:
		ic.setDirect(ic.getDirect(addr)+ic.rb, v)
	case IndirectMode:
		ic.setDirect(ic.getDirect(ic.getDirect(addr)), v)
	}
}
