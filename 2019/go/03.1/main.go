...
func main() {
	...
	if len(paths) < 2 {
		fmt.Printf("Not enough paths: %v generated", len(paths))
		return
	}

	intersections := paths.Intersections()
	if len(intersections) == 0 {
		fmt.Printf("Cannot find intersections!")
		return
	}
	fmt.Printf("Closest intersection: %v\n", intersections[0])
	fmt.Printf("Distance to the closest intersection: %v\n", intersections[0].DistanceFromCentralPort())
}
