...
impl Game {
    ...
    fn run_loop(&mut self) {
        ...
        while let Some((coord, tile)) = self.get_input() {
            match tile {
                Tile::Score(s) => score = s,
                Tile::Ball => ball = Some(coord),
                Tile::Paddle => paddle = Some(coord),
                _ => {
                    self.screen.insert(coord, tile);
                }
            }

            if let (Some(b), Some(p)) = (ball, paddle) {
                use std::cmp::Ordering;
                let command = match b.x.cmp(&p.x) {
                    Ordering::Less => -1,
                    Ordering::Greater => 1,
                    Ordering::Equal => 0,
                };
                if self.output.send(command).is_err() {
                    break;
                }
                ball = None;
            }
