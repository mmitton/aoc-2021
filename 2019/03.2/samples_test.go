...
func TestIntersections(t *testing.T) {
	...
	for _, testCase := range testCases {
			...
			if len(intersections) != len(testCase.expectedIntersections) {
				...
				for _, p := range intersections {
					if testCase.expectedIntersections.Contains(p) {
						t.Errorf("ERROR: Expected intersection %v, did not find in %v", p, testCase.expectedIntersections)
					}
				}
			}
		}
		...
		if err != nil {
			...
			if smallestDelayProp != testCase.expectedSmallestDelay {
				t.Errorf("ERROR: Paths didn't have expected smallest delay.  Expected:%v  Got:%v", testCase.expectedSmallestDelay, smallestDelayProp)
			} else {
				t.Logf("Smallest Delay Prop.  Expected:%v  Got:%v  GotAt:%v", testCase.expectedSmallestDelay, smallestDelayProp, smallestDelayPoint)
			}
		}
	}
}
