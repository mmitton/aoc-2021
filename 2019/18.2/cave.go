...
func newCave(caveMap []string) (*Cave, error) {
	...
	starting.isStart = false
	c.starting = []*Tile{
		c.GetTile(starting.x-1, starting.y-1),
		c.GetTile(starting.x-1, starting.y+1),
		c.GetTile(starting.x+1, starting.y+1),
		c.GetTile(starting.x+1, starting.y-1),
	}
	makeWall := func(t *Tile) {
		t.isPassage = false
		t.isWall = true
	}
	makeWall(starting)
	makeWall(c.GetTile(starting.x-1, starting.y))
	makeWall(c.GetTile(starting.x+1, starting.y))
	makeWall(c.GetTile(starting.x, starting.y-1))
	makeWall(c.GetTile(starting.x, starting.y+1))
	for _, starting := range c.starting {
		starting.isStart = true
	}
...
func (p *Path) newPath(np *NodePath, which int) *Path {
	p2 := &Path{}
	p2.steps = p.steps + np.steps
	p2.start = p.start
	p2.at = append(p2.at, p.at...)
	p2.at[which] = np.to
	...
	p2.nodePaths = make([][]*NodePath, len(p.start))

	p2.nodePaths[which] = append(p2.nodePaths[which], p.nodePaths[which]...)
	p2.nodePaths[which] = append(p2.nodePaths[which], np)
...
func (p *Path) String() string {
	var nodes []string
	for which, start := range p.start {
		var localNodes []string
		localNodes = append(localNodes, start.String())
		for _, np := range p.nodePaths[which] {
			localNodes = append(localNodes, np.to.String())
		}

		nodes = append(nodes, fmt.Sprintf("[%v]", strings.Join(localNodes, ", ")))
	}
...
func (p *Path) findNext(c *Cave, shortestDistance int) []*Path {
	...
	for which, at := range p.at {
		for _, next := range at.node.paths {
			...
			if hasKeys {
				ret = append(ret, p.newPath(next, which))
			}
		}
	}
...
func (c *Cave) FindShortestPaths() ([]*Path, error) {
	var workingPaths = []*Path{&Path{start: c.starting, at: c.starting, nodePaths: make([][]*NodePath, len(c.starting))}}
