...
impl Lines {
    ...
    fn search_up(file: &str, file_type: SearchType) -> Result<PathBuf, Error> {
    ...
    fn get_files(
        ...
        let input_files = Self::search_up("input_files", SearchType::Dir)?;
    ...
    fn download_input(year: usize, day: usize) -> Result<(), Error> {
        let mut local = Self::search_up("input_files", SearchType::Dir)?;
        local.push(format!("input-{year}-{day:02}.txt"));
        if local.is_file() {
            return Ok(());
        }
        let url = format!("https://adventofcode.com/{year}/day/{day}/input");
        let cookies_path = Self::search_up("cookies.txt", SearchType::File)?;
