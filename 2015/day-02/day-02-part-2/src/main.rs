...
impl Present {
    fn two_min(&self) -> (usize, usize) {
        if self.l >= self.w && self.l >= self.h {
            (self.w, self.h)
        } else if self.w >= self.h && self.w >= self.l {
            (self.h, self.l)
        } else {
            (self.w, self.l)
        }
    ...
    fn ribbon(&self) -> usize {
        let min = self.two_min();
        let min = min.0 + min.0 + min.1 + min.1;
...
fn main() -> Result<(), Error> {
    ...
    println!("wrapping_paper: {}", wrapping_paper);
