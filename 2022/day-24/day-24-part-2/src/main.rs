...
fn main() {
    ...
    let mut positions: Vec<((isize, isize), usize)> = vec![((1, 0), 0)];
    ...
    loop {
        ...
        for position in positions {
            let (position, trips) = position;
            macro_rules! new_pos {
                ($dx:expr, $dy:expr) => {{
                    if position.1 + $dy >= 0 && position.1 + $dy <= max_y as isize - 1 {
                        let mut trips = trips;
                        ...
                        let new_position = (new_pos, trips);
                        if !entities_at.contains(&new_pos) && !new_positions.contains(&new_position)
                        {
