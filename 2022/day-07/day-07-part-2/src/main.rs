...
impl Dir {
    ...
    fn part2(&self, need: usize) -> Vec<(usize, String)> {
        let mut ret = Vec::new();
        if self.size() >= need {
            ret.push((self.size(), self.name.clone()));
        }

        for dir in self.dirs.values() {
            ret.extend_from_slice(&dir.part2(need));
        }
...
fn main() {
    ...
    let space_needed = total_disk - dir.size();

    let mut possible = dir.part2(space_needed);
    possible.sort();
