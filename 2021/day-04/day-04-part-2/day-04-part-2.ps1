...
for ($i = 2; $i -lt $lines.length; $i += 6) {
    $board = @();
    for ($j = 0; $j -lt 5; $j++) {
        $board += , @($lines[$i + $j].trim() -replace '\s+', ' ' -split " ")
    }
...
foreach ($called_number in $called_numbers) {
    foreach ($idx in (0..($boards.length - 1))) {
        if ($boards[$idx] -ne $null) {
            $win, $ans = Play-Move $boards[$idx] $called_number
            if ($win -eq $true) {
                Write-Host "Number $called_number let board $idx win with answer: $ans"
