...
fn find_beacons(
    ...
    fn iter(
        ...
        if found == scanners.len() {
            ...
            let mut final_pos: Vec<Point> = Vec::new();
            for p in &positions {
                final_pos.push(p.unwrap().1);
            }
            beacons.sort();
            return Some((beacons, final_pos));
        }
...
fn main() -> Result<(), Error> {
    ...
    let result = find_beacons(&scanners, &overlaps);
    if let Some((beacons, final_pos)) = result {
        println!("Beacons: {}", beacons.len());
        for beacon in &beacons {
            println!("  {:?}", beacon);
        }
        println!("Final Positions: {}", final_pos.len());
        for pos in &final_pos {
            println!("  {:?}", pos);
        }

        let mut best = 0isize;
        for i in 0..final_pos.len() {
            for j in i + 1..final_pos.len() {
                let x_diff = (final_pos[i].x - final_pos[j].x).abs();
                let y_diff = (final_pos[i].y - final_pos[j].y).abs();
                let z_diff = (final_pos[i].z - final_pos[j].z).abs();

                let dist = x_diff + y_diff + z_diff;
                if dist > best {
                    best = dist;
                }
                println!("dist between {} and {} = {}", i, j, dist);
            }
        }
