...
impl Map {
    ...
    fn get_risk(&self, x: usize, y: usize) -> u8 {
        if x >= self.0[0].len() * 5 || y >= self.0.len() * 5 {
            panic!("Out of bounds");
        }

        let x_mul = x / self.0[0].len();
        let x_off = x % self.0[0].len();

        let y_mul = y / self.0.len();
        let y_off = y % self.0.len();

        let dist = (x_mul + y_mul) as u8;
        ((self.0[y_off][x_off] + dist - 1) % 9) + 1
    }
    ...
    fn find_shortest_path(&self) -> Path {
        ...
        for _ in 0..self.0.len() * 5 {
            lowest_risk.push(vec![!0usize; 5 * self.0[0].len()]);
        }

        let max_x = (5 * self.0[0].len()) - 1;
        let max_y = (5 * self.0.len()) - 1;
        lowest_risk[0][0] = 0;
        ...
        while paths.len() > 0 {
            ...
            if x != max_x || y != max_y {
                for m in &[(-1isize, 0isize), (1, 0), (0, -1), (0, 1)] {
                    ...
                    let cell_risk = self.get_risk(new_x, new_y);
...
fn main() -> Result<(), Error> {
    ...
    println!("shortest_path: {:?}", shortest_path.risk);
